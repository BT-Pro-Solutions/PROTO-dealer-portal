...
import { fetchTenantById, updateTenantFeatures, featureLabels } from '../../data/admin.js'
...
onMounted(async () => {
  ...
  features.value = { ...tenant.value.features }
})
...
async function handleSubmit() {
  ...
  try {
    tenant.value = await updateTenantFeatures(tenant.value.id, features.value)
    saveMessage.value = 'Features saved.'
  } catch {
    saveMessage.value = 'Unable to save features.'
  } finally {
    saving.value = false
  }
}
...
<template>
  <div v-if="tenant" class="tenant-form page-content page-content--narrow">
    ...
    <h1 class="tenant-form__title reveal reveal--delay-1">Feature flags</h1>
    <p class="tenant-form__subtitle reveal reveal--delay-1">
      Control tenant-level portal capabilities for {{ tenant.name }}.
    </p>
